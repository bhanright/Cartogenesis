...
### F35. The same seed makes a different world at each resolution — on `main`
...
big the planet is. Map sizes and shapes beyond the square grid are a separate question, to be
discussed with the maintainer before anything is written down. Regions with a limited latitude range are deliberately out of scope: they
...
| F35 The same seed makes a different world at each resolution | | done | 2026-09-19 | 253ea1b on chunk/f35-resolution-invariant-seeds | **A plate seed's place is now drawn on the reference grid and read as a fraction of whatever grid is generated, so 512's world is the world at every size.** Cause confirmed before the change, on seeds 217862 and 718106 at 512, 1024 and 2048: Kotlin's `nextInt(until)` takes the generator's top bits for a power-of-two bound and falls back to a modulo otherwise, so the column `nextInt(width)` agreed to within 0.75 cells of the 512 grid at every size while the row, whose bound is the height less the pole margins and so never a power of two, disagreed by up to 0.737 of the grid — 377 cells of 512, every plate at a different latitude and the whole partition with it. Fix: the two draws are made once against `SEED_REFERENCE_CELLS` (512, `WorldGenConfig`'s own default and what `atResolution` refines from) and the cell each names is placed by its middle as a fraction of the working grid; the draws, their order and their count are unchanged, so the 512 world is bit-identical to main and every other grid is that world drawn finer. Measured after: a seed sits 0.50 cells of the coarse grid from itself at 1024 and at 2048 on all four standard seeds. **The audit found no other draw to convert**: these two were the only `nextInt(bound)` in the generator whose bound came from the width, the height or a count of cells; `NameForge`'s syllables, realm forms and name salts are counts of things, `PerlinNoise`'s is a fixed 256-entry permutation, `PlateStage`'s volcano offsets and the hearth, realm, landmark and schism jitters are already `nextFloat()`, and the crust shuffle picks among plates. Reported and not changed: `LandmarkStage` draws one jitter per *land cell* from a single stream, so which cell gets which jitter depends on how many cells there are — resolution-dependent by construction, a decoration on a score rather than a bound, and its own chunk if it is worth one. **Guards** in `ScaleFreeTest`, each shown failing on the old draw and passing after: the plate seeds' fractional positions agree at 512, 1024 and 2048 to within one cell of the coarsest (290–404 cells before, 0.50 after); the plate-id field at 1024, read at the fine cell nearest each coarse one, matches the 512 field everywhere further than one coarse cell from a boundary (0.139–0.257 of the interior before, 0.99998–1.00000 after, bar 0.999 with its derivation beside it — a seed may sit a cell from where the other grid puts it, so a bisector may, and the domain warp carries that cell into the map; the deepest disagreement measures 1.41 coarse cells from a boundary); and the land mask after sea level the same way, on every cell the shore does not touch, that share computed from the 512 coast rather than chosen (0.445–0.727 before against bars of 0.952–0.962, 0.981–0.984 after). **One re-pin**, because 512 did not move: `DepositionTest`'s starting-point land at 128, 6279 → 6273, the sixteenth time that figure has moved and the reason written into its list. `FORMAT_VERSION` unchanged, no field changed, the codec fixture untouched. **Renders** at `desktop/build/f35-crops/{before,after}/`: before, 718106's 1024 and 2048 are plainly two different planets; after, both seeds read as one world at two sizes — the same snowy range and south-facing bays on 718106, the same triangular northern continent, gulf and ring-lagoon archipelago on 59758 — and nothing reads worse, the finer grid only resolving more fjords and lakes. **Two Earth bars the new worlds do not meet, reported not moved, and the same finding twice**: the worlds at grids other than 512 are now different worlds, and two Earth-derived bars the old scrambled worlds happened to meet are not met by the true ones. Both pass on main; both bars are Earth figures, which ground rule 5 forbids moving to fit a measurement; so M1's rule applies and each clause keeps asserting every case that passes today and prints the one that does not with Earth's figure beside it, the finding asserted non-empty so it cannot go quiet. `GlacialBasinShapeTest`'s floor clause: 718106 at 1024 carries a 167-cell cut basin at (488,25) whose floor is 78.4% within a metre of one height over 51 m of relief against the 54.9% Salar de Uyuni allows — an I2-class flat-floored basin on ground the guard had never looked at, queued as I2b. `OutletResolutionTest`'s pooled clause: 59758 at 2048 reads 1.84 times the Caspian's share of Earth's land (0.4577% of its land against Earth's 0.2491%) and is left out of the pool, which then reads 0.98 times over five worlds where all six read 1.12; queued as F37. Both are in TODO.md with the figures and with what a diagnosis should notice first — the 512 figures did not move at all (59758 1.55x, 42 0.67x, as on main), so whatever these are, they are something the finer grids resolve. Tier green: worldgen 180, cartography 54, ui jvm 115, ui wasm 115, desktop 75, siteTest 18, none failed. |
| I2b A flat-floored basin at 718106 1024 | | queued (found by F35, 2026-09-19) | 2026-09-19 | | 167-cell cut basin at (488,25), 78.4% of its floor within a metre over 51 m of relief against 54.9%; GlacialBasinShapeTest's floor clause is a finding on that case until fixed |
| F37 The largest lake is 1.12x the Caspian's share | | queued (found by F35, 2026-09-19) | 2026-09-19 | | Pooled over six worlds after the plate seeds became resolution-free; 59758 1.84x at 2048; OutletResolutionTest's pooled clause is a finding until fixed |
