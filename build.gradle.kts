...
/*
 * The per-merge tier's share of the machine: how many test workers each JVM test task forks, and
 * how many threads each worker's common pool may use, decided here because they are one budget.
 *
 ...
 * starts one; left alone, six workers would each run a pool as wide as the machine. So each worker
 * is given a pool of its own width, and the widths add up to the hardware threads:
 *
 *   :worldgen:jvmTest     3 workers x 4 threads, 4 GB heap each       12 threads, 12 GB
 *   :desktop:test         1 worker  x 2 threads, 4 GB heap             2 threads,  4 GB
 *   :cartography:jvmTest  1 worker  x 1 thread,  3 GB heap             1 thread,   3 GB
 ...
 * doing. Generation is mostly serial — a lone worker with the whole pool kept the processor under
 * a quarter busy — so a narrower pool costs a worker little and three workers gain three times.
 * The figures scale down with the machine, never below one worker and two threads a generating
 * worker, so a four-core runner gets one worldgen worker rather than three contending for it.
 *
...
extra["worldgenTestForks"] = (hardwareThreads / 5).coerceIn(1, 3)
extra["worldgenTestPoolThreads"] = (hardwareThreads / 4).coerceAtLeast(2)
extra["worldgenTestHeap"] = "4g"
extra["desktopTestPoolThreads"] = (hardwareThreads / 8).coerceAtLeast(2)
extra["desktopTestHeap"] = "4g"
extra["cartographyTestHeap"] = "3g"
