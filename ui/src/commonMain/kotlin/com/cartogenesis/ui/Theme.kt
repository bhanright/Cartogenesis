 ...
 * contrast, colour-blind, Allied, Hallowed, Baroque, Matrix, Hessian, Roman, Hitchcock, Lemon
 * Blueberry, Blacklight — and each is a room rather than a chart. Seventeen is more than a flat
 * list can carry, so [group] puts them on three shelves; nothing about a name or a stored value
 * moves.
 *
...
enum class ThemeChoice(val label: String) {
     ...
     *
     * See [ChromeDetail]. Eleven of the seventeen answer with something other than the default;
     * the six plain ones answer with the default itself, which is what keeps their screenshots
     * pixel-identical.
     */
...
// ---- One room made of a fruit. ----

// LEMON BLUEBERRY: the two colours the name says, and the question of which way round they go.
//
// The author asked for the pair and left the rest open, so the arrangement was decided by measuring
// rather than by taste. Both were built to the same rules — the same two families of colour, the
// shared armed button (a wash of the ground stained toward the accent, with the accent for a label,
...
/** The stain under an armed control: the shared wash, in the only colour this room has. */
...
// ---- One room lit by a lamp. ----

// BLACKLIGHT: #E6FF42 and #520C94, and the same question Lemon Blueberry asked of its two.
//
// The author gave the pair and nothing else, so which of them is the room was decided by
// measuring. Both arrangements were built to one set of rules — the named colour is the panel a
// word is read on, the window and the sunk well are that same colour a step either side of it, the
// ink is the *other* colour taken as far from the panel as the panel's own room allows, the accent
// is the other colour at full strength, and the armed button is the shared stain every chrome that
// does not invert uses: the panel taken about ten L* toward the ground, never an inverted block.
// Every text pair in each was then measured against WCAG AA, and the armed Stop button with them:
//
...
/** The room past the lamp's reach: the given violet at half its lightness, L* 24.3 down to 10.6. */
private val VioletGround = Color(0xFF2B064F)

/** The panel, and it is the given violet exactly. Every word in the chrome is read on this. */
 ...
 *
 * The shared wash rather than a block, and ten L* below the panel because a stain darkens what it
 * soaks into — which is the whole of why this arrangement's Stop button measures 13.97:1 and the
 * other's 7.73:1.
 */
...
/** The highlighter at full strength, and it is the given lime exactly. 10.19:1 on a panel. */
