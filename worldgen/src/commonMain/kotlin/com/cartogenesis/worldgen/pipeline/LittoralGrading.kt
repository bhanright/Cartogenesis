 ...
 * order.
 *
 * The generator models the drowning and not the tidying up. [SeaConfig.lowstand] drops the base
 * level for nine of the twelve hydraulic rounds, so every cell within about 120 m of the shoreline
...
internal object LittoralGrading {
     ...
     * [SeaLevelResult.relativeElevation] units.
     *
     * The postglacial rise itself: 120 m against the eight kilometres of relief between sea level
     * and the highest land, which is the 1.5% [SeaConfig.lowstand] holds, for the same reason. What
     * a coast's plan form records is what the transgression found when it arrived. Where the land
     * within reach of the shore stands lower than the rise, the sea came in across a flat and has
     * been filling it ever since — the Dutch coast is under 10 m for its first fifty kilometres,
     * the Texas coast under 30, Bengal under 10. Where it stands higher, the sea ran up valleys and
     * the outline is the valleys' — Galicia is 200 to 400 m within fifty kilometres of the water,
     * western Norway over 500.
     *
     * Held here rather than read from [SeaConfig.lowstand] because that setting is a switch as well
     * as a figure: the estuary guard turns it off to get its control, and how a coast is shaped does
     * not stop depending on the height of the land behind it when it does.
     */
    const val MAX_BACKSHORE_RISE = 0.015f
     ...
     * as an invariant. The reach is [REACH_KILOMETRES], and one metre per kilometre over that reach
     * is 23 m — 0.003 of the land's relief, and so always less than [HydraulicErosion.POND_DEPTH].
     * A coastal flat this pass lays can never stand high enough above the waterline to hold standing
     * water, at any resolution.
     */
    const val COASTAL_PLAIN_METRES_PER_KILOMETRE = 1f

    /** The relief between sea level and the highest land, in metres. See [SeaConfig.lowstand]. */
    const val LAND_RELIEF_METRES = 8000f

    /**
     * How wide the map is taken to be, in kilometres.
     *
     * The same 12,000 km `NationsConfig.worldWidthKm` uses, repeated rather than read: the sea
     * stage's reuse guard would otherwise have to name the nations section, and moving the realm
     * count would re-cut the sea.
     */
    const val WORLD_WIDTH_KILOMETRES = 12_000f

    /**
     * How far along the shore the littoral system carries sediment, in kilometres.
     *
     * Twenty-three, which is one cell of a 512 world and the distance [SeaConfig.littoralReach]
     * holds as a fraction of the map. Earth's reference is the spacing of the inlets through a barrier coast, because that
     * spacing is the length of shore a drift system holds unbroken: the Frisian islands run 10 to
     * 30 km between gaps, the Outer Banks 20 to 60, Padre Island 180 in one piece. Nothing narrower
     * than the shortest of those survives on a coast with drift, so nothing narrower than that
     * survives here. Recorded as a constant because the config carries the same distance in cells
     * and the two have to be read together.
     */
    const val REACH_KILOMETRES = WORLD_WIDTH_KILOMETRES / 512f
     ...
     * all agreeing with the graded shoreline. Deterministic: it reads the cut and [sea] and nothing
     * else, so the engine's stage reuse gets the same answer as a fresh generation.
     *
     * [landRelief] and [seaRelief] are the height field's own spans above and below the shoreline,
     * which are what turn the one figure this pass is bounded by — the postglacial rise — into a
     * depth in the water's units as well as a height in the land's. See [floodedDepthLimit].
     */
    fun apply(
        cut: SeaLevelResult,
        sea: SeaConfig,
        ...
        val reachCells = (sea.littoralReach * cellsAcross).roundToInt()
        if (!sea.littoralGrading || reachCells < 1) return cut

        val cellCount = cellsAcross * cellsDown
        if (cut.landCellCount == 0 || cut.landCellCount == cellCount) return cut

        val sweeps = gradingSweepsPerCell(cut, sea, reachCells, floodedDepthLimit(landRelief, seaRelief))
        ...
        val risePerCell = COASTAL_PLAIN_METRES_PER_KILOMETRE *
            (WORLD_WIDTH_KILOMETRES / cellsAcross) / LAND_RELIEF_METRES
        ...
        // the sweeps filled.
        return SeaLevelResult(cut.threshold, isLand, relative, cut.landCellCount + built)
    }
     ...
     */
    fun depositionalShareOfShoreline(
        cut: SeaLevelResult,
        sea: SeaConfig,
        landRelief: Float,
        seaRelief: Float
        ...
        val cellsAcross = cut.relativeElevation.width
        val cellsDown = cut.relativeElevation.height
        val reachCells = (sea.littoralReach * cellsAcross).roundToInt().coerceAtLeast(1)
        val sweeps =
            gradingSweepsPerCell(cut, sea, reachCells, floodedDepthLimit(landRelief, seaRelief))
        var shoreline = 0
        var depositional = 0
     ...
     *
     * The same [MAX_BACKSHORE_RISE] the land is judged by, read downwards, which is why it takes
     * both spans: a height field's relief above the shoreline and its depth below it are different
     * numbers, and the 120 m is a distance on the ground rather than a fraction of either.
     */
    private fun floodedDepthLimit(landRelief: Float, seaRelief: Float): Float =
        -MAX_BACKSHORE_RISE * landRelief / seaRelief.coerceAtLeast(1e-6f)
     ...
     * Two were tried. The postglacial rise, [MAX_BACKSHORE_RISE] — did the sea flood a flat or run
     ...
     *
     * So Earth's figure goes in directly instead, the way `SeaConfig.enclosedSeaMaxShare` carries
    ...
    private fun gradingSweepsPerCell(
        ...
        val backshoreCells = (sea.littoralBackshore * cellsAcross).roundToInt().coerceAtLeast(1)
        val fetchCells = (sea.littoralFetch * cellsAcross).roundToInt().coerceAtLeast(1)
