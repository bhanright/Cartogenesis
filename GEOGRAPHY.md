...
**The cold-air moisture cap leaves high-latitude west coasts as taiga/tundra despite wet conditions.** The `coldCap` clamps moisture by annual mean temperature; at 50–60° a warm current shore still has a cold annual mean, so the cap prevents the winter rains from supporting forest. Measured on seeds 7, 42, 1234 at 512×512: zero cells classed as temperate rainforest or temperate forest on 50–60° west-facing coasts, despite 1.88–3.83× the latitudinal mean precipitation. The coasts are half warm-current (anom +1.7–2.0°C) and half cold-current, yet both are locked in taiga/tundra. Confirmed problem — follow-up: use winter temperature instead of annual mean in the cap, since moisture-holding capacity is set by the season when it rains.
