...
| F36 Linux download, apt repository and installation instructions | | done | 2026-09-19 | chunk/f36-linux | **A Linux release workflow, a signed apt repository under the site, and one set of installation instructions three documents read from.** `.github/workflows/release-linux.yml` on a `v*` tag: ubuntu-latest, JDK 21, `fakeroot`, `:desktop:test`, `createDistributable` and `packageDeb`, then the packaged launcher's `--gpu-check` on a runner with no card. **That check needed no fix** — `GlContext.ensure` catches the failure through `Future.get`'s `ExecutionException`, reports "unavailable" and returns, and the GL worker is a daemon thread, so finding no device already exits 0; the step reads the code and explains a non-zero one rather than assuming. It then names `Cartogenesis-<version>-linux-amd64.deb` and `Cartogenesis-<version>-linux-portable.tar.gz` from `gradle.properties` (warning, not failing, if the tag disagrees), **waits up to thirty minutes for the release to exist** — it is cut by hand from a Windows machine, and may appear minutes after the tag — and uploads both with `gh release upload --clobber`. **The pool is rebuilt from the releases each deploy, not committed back to main**: a `.deb` is about 100 MB, so a committed pool grows the repository by that much per release for ever; the releases become the one source of truth; and a workflow that pushes to `main` can collide with a human push on release day. It costs a few minutes of downloading per deploy. **The rebuild had to move into `site.yml`**, which is the decision worth recording: a Cloudflare Pages Direct Upload publishes a *whole tree*, so a site deploy that did not carry `apt/` — a copy change, a hand-run — would take the repository off the domain until the next release. Building it in the deploy means every deployment carries a complete repository whatever triggered it; `release-linux.yml`'s second job only asks for a deploy once the `.deb` is on the release, and the site workflow's serial, uncancelled concurrency group puts that run last. It writes into `web/build/site/apt` rather than `site/` for a second reason found on the way: every `:desktop:` test task declares the whole of `site/` as an input, so a pool dropped there would be hashed by Gradle on every run. `site/apt/conf/distributions` is the only committed part, with `Limit: -1` (reprepro keeps one version by default, which on a from-scratch rebuild would leave only the newest release in the pool) and a `__APT_SIGNING_KEY_ID__` the deploy substitutes; reprepro signs through `gpg-agent` in its own process, so loopback pinentry is allowed and one throwaway signature primes the cache. A missing secret is an `::error::` naming it and a site that deploys without `/apt`, never a failed release. `_headers` caches `/apt/pool/*` for a year and `/apt/dists/*` not at all. **One list, three readers**: `site/downloads.txt` names every release file with `<version>` in it — chosen over a list in `desktop/build.gradle.kts` because it is data about a release rather than a build rule, and it is excluded from the published tree along with `apt/conf/**`. The real names were read off the release page rather than guessed: the installer is `Cartogenesis-<version>.msi`, not `-windows.msi`. `SiteAssemblyTest` gains three cases holding the page's new `id="install"` section, `docs/INSTALL.md` and `docs/RELEASE_NOTES_TEMPLATE.md` to that list in both directions, checking the apt source line matches word for word across all three and ends in the codename `conf/distributions` actually builds, and reading the hero button's label for both platform names — it read "Download for Windows (recommended)" and would have survived a Linux download perfectly happily. **The page**: `id="install"`, "Download and Installation", after Notes and before the roadmap, four panels in the Notes grid (Windows, Linux with the three commands in a new `.cmd` block, Browser, the release page), no new palette variable — `bone` on `ink-sunk` and `bone` on `ink-raised` added to `SitePaletteContrastTest`, 11.9:1 and 10.4:1. The hero button reads "Download the Desktop App (Windows/Linux)" and points at the new section; the Notes card's Windows tag becomes Windows · Linux and its SmartScreen sentences move into the section rather than being kept in two places. All copy is the maintainer's final pass, applied verbatim; no placeholder survives. **`docs/INSTALL.md`** carries the long form per platform, where saves live (`~/.cartogenesis/worlds`, `/opt/cartogenesis/bin/Cartogenesis` with no `/usr/bin` symlink), the graphics note, and a numbered walkthrough of `gpg --full-generate-key`, the key id, the armoured export and the three secrets. **README** gains a build-prerequisites table, one row per platform, and stops saying no Linux download is published. **Gates**: `:desktop:test` 75 green, `:desktop:siteTest` 21 green (18 before), both workflows parse. The release workflow was not run. |
