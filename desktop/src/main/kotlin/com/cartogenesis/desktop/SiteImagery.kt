 ...
 * the page shows.
 *
 * Generating [SEED] at [SIZE] is a minute of arithmetic and a figure is a window onto that map,
 * cropped at the render's own pixels. Cropping rather than scaling is the point: every mark the
 * renderer makes is sized in *output pixels* (F9's lesson, and F10's river pen), so a 1:1 window
...
object SiteImagery {
     ...
     * map only works when a person wrote it (2026-09-12). [detail] may be empty where there is
     ...
     * wrote them, and took the six plain cards back in their place. The two strips below are the
     * comparison he asked for on 2026-09-14: the three styles that draw the world as a physical
     * map, and the four layers the map is drawn from.
     *
