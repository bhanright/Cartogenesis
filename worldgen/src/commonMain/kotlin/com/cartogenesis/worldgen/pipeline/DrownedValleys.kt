...
internal object DrownedValleys {
     ...
     * side of a channel in the same units as the channel's own floor.
     *
    ...
    fun apply(
        ...
        if (!sea.drownedValleyFill) return cut
        ...
        //
        // The rise as a figure, [LittoralGrading.MAX_BACKSHORE_RISE], rather than
        // [SeaConfig.lowstand] as a setting, and the difference matters. That setting is a switch as
        // well as a number: `SeaLevelHistoryTest` turns it off to get the control it compares the
        // drowned coast against. Reading it here would leave the control with every notch this pass
        // exists to remove and take them all off the arm under test, which is not a comparison of
        // anything — measured, it took seed 7's indentation from 5.20 in the control to 4.59 in the
        // arm and inverted the claim the guard makes.
        val transgression = threshold - LittoralGrading.MAX_BACKSHORE_RISE * landRelief

        val drowned = drownedNotches(cut, height, transgression, oceanOf(cut))
        if (drowned.isEmpty()) return cut

        val arriving = catchmentArrivingByCell(cut, cellCount, seed, byFacet)
        carryCatchmentDownTheValleys(drowned, arriving, cut.isLand, height)

        val cellWidthKilometres = LittoralGrading.WORLD_WIDTH_KILOMETRES / cellsAcross
        ...
        val filled = raiseTheValleyFloors(
            unresolved, cut, height, isLand, relative, threshold, landRelief, channelShareOfCell
        )
    ...
    private fun raiseTheValleyFloors(
        ...
        val oneMetre = landRelief / LittoralGrading.LAND_RELIEF_METRES
