...
- **JPEG** as a third picture format, for the tools that still refuse WebP: quality about 90,
  no alpha, the JDK's encoder on the desktop and the canvas encoder on the web, one more chip in
  the format row. Not for quality — WebP is smaller at the same quality and keeps thin lines
  cleaner — but for compatibility, and the note beside the chip says so.
...
| M1 Earth-likeness metric suite | Opus | in progress on main | 2026-09-12 | | |
