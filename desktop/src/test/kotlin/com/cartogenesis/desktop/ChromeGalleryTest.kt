...
class ChromeGalleryTest {
     ...
     *
     * Asserted here is what a screenshot allows and no more: that the two pictures differ, and
     * that neither came out flat.
     */
    @Test
    ...
    /**
     * The other two-colour chrome, at 1440x900 and on a phone.
     *
     * The same shot as Lemon Blueberry's and for the same reason — Blacklight is a chrome of two
     * colours and no ornament, so what a picture has to answer is whether two colours are enough
     * to tell a panel from the ground it is on, an accent from the ink and an alarm from either.
     * It asks one thing the other did not, though: this chrome's panel is a saturated violet at
     * chroma 79 rather than a near-black, and a large field of saturated colour behind small type
     * is the kind of thing that measures well and reads badly. The phone shot is where that would
     * show worst, since at 390 dp the panels become a sheet pulled over the map and there is more
     * of them than there is map.
     *
     * Recorded alongside the named, typographic and Lemon Blueberry shots rather than added to any
     * of them, so no existing capture moves.
     */
    @Test
    fun `the Blacklight chrome is photographed at a desk and on a phone`() {
        ...
        println(
            "CHROME wrote the ${WIDTH}x$HEIGHT and 390x844 Blacklight shots to " +
                "${dir.absolutePath}; " +
                "fingerprints wide ${wide.fingerprint}, phone ${down.fingerprint}, " +
                "sheet ${up.fingerprint}"
        )
