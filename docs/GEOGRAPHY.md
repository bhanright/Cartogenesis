...
**The land's outlines are consistent with cell-space anisotropy in the kilometre metric: they run about twice as far east-west as north-south.** A square grid over a world twice as wide as it is tall has cells twice as wide as they are tall, and a landscape isotropic on the ground would put as much of its coastline's length into east-west travel as into north-south. It does not. Over seven worlds (969495 and the six audited seeds) the coastline projects 1.93, 1.99 and 2.06 times as far east-west as north-south at 512, 1024 and 2048, 1.82 to 2.14 by world, and the 2,000 m contour 1.99, 2.00 and 2.04, 1.83 to 2.24; `CoastalSpacingAuditTest` prints both for every world. That is what land isotropic in *cells* would give. Two inputs that could draw it that way are known, and neither is new — the terrain noise is sampled on a lattice with as many cycles down the map as across it, and `PlateStage` measures every belt's distance from its boundary in cells with no row scale (`TODO.md` already carries the second) — and which of them carries it, or whether something else does, is not measured. It is why X1d's finder, isotropic in kilometres, found one to three straight north-south coasts against 86 to 121 east-west ones at each grid, and why a spacing's ratio between the two bearings could not tell the ground from the grid. The routing reads its steps on the same square ruler: on a plane falling equally per kilometre east and south it routes 14.1 degrees south of east where the ground falls at 45. Correcting either would reshape every world, so it is a question for the terrain's and the routing's own chunks and not for a measurement.
