...
val generateNotices = tasks.register("generateNotices") {
    ...
    // The two faces are bundled as Compose resources rather than resolved as dependencies, so they
    // appear in no graph — and they are the two notices that are a licence requirement rather than
    // a courtesy. The OFL text beside them is what is read here.
    ...
    doLast {
        ...
        listOf(ofl("Spectral", spectral), ofl("IBM Plex Sans", plex))
        ...
        dir.resolve("Notices.kt").writeText(
            buildString {
                ...
                appendLine(" * the two bundled type faces, with whatever licence each POM declares. Read by")
