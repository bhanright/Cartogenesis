...
**High-latitude west coasts classify as taiga/tundra despite abundant rainfall.** Measured on seeds 7, 42, 1234 at 512×512: zero cells on 50–60° west-facing coasts class as temperate forest despite 3.83–3.71–1.88× the latitudinal mean precipitation (seeds 42/7/1234). The cold cap is not the cause — rainfall is abundant (coast precip 0.91–0.99 normalized). The cause is `classify`, which gates temperate/taiga/tundra on annual-mean temperature (< 7 °C → taiga), and the latitude curve places 55° near 0 °C, so warm-current anomalies (+1.7–2.0°C) still fall below 7 °C. Bergen is temperate by Köppen definition (coldest month > −3 °C, warmest > 10 °C), not by annual mean. This will be addressed when A1's seasonal fields enable Köppen-style classification on seasonal extremes rather than annual means; the latitude curve may also run too cold at 45–60°.
