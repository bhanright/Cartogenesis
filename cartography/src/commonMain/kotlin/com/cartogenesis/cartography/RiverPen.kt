 ...
 *
 * So the full stroke is a *share of the map*, not a count of pixels. F10 held it at five pixels at
 * every size, which William called immensely better at 2048 and twice too heavy at 1024 — the same
 * country, the same rivers, half the sheet, the same ink. A share gives the same picture at every
 * size: two and a half pixels at 1024, five at 2048, ten at 4096, and a map printed twice as large
 * has rivers twice as wide, exactly as a map printed twice as large has everything else.
 *
 ...
 * channel. F9's hachures are the same kind of thing and stay in pixels for the same reason; see
 * [Engraving].
 *
...
object RiverPen {
     ...
     * map exaggerates a river of that class about threefold, which is 0.25%. F10's five pixels on
     * a 2048 sheet is 0.244% of the same width, arrived at by eye and agreeing to the second digit.
     * Both round to the figure used here.
     */
