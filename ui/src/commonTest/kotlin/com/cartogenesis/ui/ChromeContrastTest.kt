 ...
 * reader would choose it *because* of the promise. F7's four, F24's one and F28's one make a
 * quieter promise and the same kind: a chrome is a room somebody works in for an hour, and one
...
class ChromeContrastTest {
    ...
    private companion object {
         ...
         *
         * The two colours F28 was given, reduced to the one property the chrome holds every tone
         * to. See the guard that reads them.
         */
    ...
    fun `Blacklight puts the violet underneath, which is the arrangement that measured`() {
        ...
        // And the panel is the violet William named, not a tone derived from it.
        assertEquals(
            Color(0xFF520C94),
            scheme.surface,
     ...
     * colour's own hue, since nothing before F28 had a use for one. The transform is the standard's
     * and is the same one that function is built on, written out here rather than added to the
     * library for a single guard.
     */
