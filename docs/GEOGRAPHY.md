...
**A coast's valleys are spaced on the ground, except where the slope behind it is smooth and steep, and there the grid spaces them.** Measured by X1d over seven worlds (969495 and the six audited seeds) at 512, 1024 and 2048, on every straight coast at least 300 km long, with a finder that reads the routing and never the traced rivers. The valleys that reach the main divide are about 70 km apart at every grid — 70.3, 70.2 and 66.2 km, which is 3, 6 and 12 cells — and matched coast by coast the fine grid's spacing is 0.92 to 1.01 of the coarse grid's in kilometres, where a spacing set by the grid would read 0.25 to 0.5. The comb between them holds nearly as well: catchments large enough to carry a drawn course, 1,440 km2 by Hack's law, are 70.0, 58.2 and 47.8 km apart (0.83 to 0.97 coast by coast), and the traced courses — every one of which an export drew before X1c's selection — 75.8, 53.3 and 45.8 km. That last figure is 8.5 cells at 2048, which is the "ten cells" the author read, and it is a spacing on the ground seen at the grid he looked at. The exception is the author's own coast, on the eastern peninsula of 969495: a smooth slope falling from a plateau at 1,100 to 1,900 m to the sea over 100 to 150 km, beside a transform boundary and on no mountain front. There the catchments are 62 km apart at 512 and at 1024 and 19.4 km apart at 2048, and at both of the finer grids they are 2.6 cells wide — 30 km, then 15 — so the comb the author saw holds in cells, which is the grid's reading. Halving and doubling the relief wavelength, the belt width and (at 2048) the finest octave of the terrain noise, on two seeds, leaves the 2048 coasts' valley spacing at 0.82 to 1.27 of the stock world's and the catchment comb at 0.81 to 1.14, in no consistent direction, where a figure that tracked the setting would halve under one change and double under the other; the traced comb scatters wider, 0.56 to 1.29, again with no direction. The spacing of the valleys that reach the divide also does not follow the depth of the ground behind the coast the way Earth's does: Hovius (1996) found the half-width of a linear mountain belt 2.1 times its outlet spacing, and here that ratio is 0.86 to 1.05 on coasts and 0.61 to 0.95 on mountain fronts, with spacing barely rising with half-width at all. What sets the 70 km is not established. The likeliest reading of the exception is that the hillslope process here is a slope threshold, which has no length of its own: the spacing of first-order valleys on Earth is set by the ratio of hillslope transport to river incision (Perron, Kirchner and Dietrich 2009), and where the ground is smooth enough that nothing else intervenes, the cell is the only length left. That is a hypothesis, recorded in `TODO.md` with the fix it would imply and this measurement as its guard.

**The land is drawn isotropic in cells, so in kilometres its outlines run twice as far east-west as north-south.** A square grid over a world twice as wide as it is tall has cells twice as wide as they are tall, and a landscape isotropic on the ground would put as much of its coastline's length into east-west travel as into north-south. It does not. On 969495 the coastline projects 1.82 times as far east-west as north-south at 512 and 2.12 times at 2048, and the 2,000 m contour 1.99 and 2.09: the land is isotropic in *cells*. Two inputs that draw it that way are known, and neither is new — the terrain noise is sampled on a lattice with as many cycles down the map as across it, and `PlateStage` measures every belt's distance from its boundary in cells with no row scale (`TODO.md` already carries the second). It is why X1d's finder, isotropic in kilometres, found one to three straight north-south coasts against 86 to 120 east-west ones at each grid, and why a spacing's ratio between the two bearings could not tell the ground from the grid: the ground itself reads the same cells both ways. Correcting it would reshape every world, so it is a question for the terrain's own chunk and not for a measurement.
